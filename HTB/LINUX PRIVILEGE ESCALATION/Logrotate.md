...
* Furthermore, searching through many large log files is time-consuming. To prevent this and save disk space, `logrotate` has been developed. T
* he logs in `/var/log` give administrators the information they need to determine the cause behind malfunctions. Almost more important are the unnoticed system details, such as whether all services are running correctly.
...
```shell-session
canklesmaks@htb[/htb]$ man logrotate
canklesmaks@htb[/htb]$ # or
canklesmaks@htb[/htb]$ logrotate --help
...
```shell-session
canklesmaks@htb[/htb]$ cat /etc/logrotate.conf
...
```shell-session
canklesmaks@htb[/htb]$ sudo cat /var/lib/logrotate.status
...
Logrotate
	cd lo
```shell-session
canklesmaks@htb[/htb]$ nc -nlvp 9001
