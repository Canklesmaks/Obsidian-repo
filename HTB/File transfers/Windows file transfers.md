...
*  [md5sum](https://man7.org/linux/man-pages/man1/md5sum.1.html), a program that calculates and verifies 128-bit MD5 checksums.
* we can confirm if the file was transferred successfully using the [Get-FileHash](https://docs.microsoft.com/en-us/powershell/module/microsoft.powershell.utility/get-filehash?view=powershell-7.2) cmdlet.
...
![[Pasted image 20240314152621.png]]This can be bypassed using the parameter `-UseBasicParsing`.
...
```shell-session
canklesmaks@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare
...
```shell-session
canklesmaks@htb[/htb]$ sudo impacket-smbserver share -smb2support /tmp/smbshare -user test -password test
...
```shell-session
canklesmaks@htb[/htb]$ sudo pip3 install pyftpdlib
```
#### Setting up a Python3 FTP Server

```shell-session
canklesmaks@htb[/htb]$ sudo python3 -m pyftpdlib --port 21
...
**We can use the same methods we used for download operation but now for uploads. Let's see how we can accomplish uploading files in various ways.
...
```shell-session
canklesmaks@htb[/htb]$ pip3 install uploadserver
...
```shell-session
canklesmaks@htb[/htb]$ nc -lvnp 8000
...
```
```shell-session
canklesmaks@htb[/htb]$ echo <base64> | base64 -d -w 0 > hosts
```
## SMB Uploads
...
```shell-session
canklesmaks@htb[/htb]$ sudo pip install wsgidav cheroot
...
```shell-session
canklesmaks@htb[/htb]$ sudo wsgidav --host=0.0.0.0 --port=80 --root=/tmp --auth=anonymous 
...
```shell-session
canklesmaks@htb[/htb]$ sudo python3 -m pyftpdlib --port 21 --write
...
 [PSUpload.ps1](https://github.com/juliourena/plaintext/blob/master/Powershell/PSUpload.ps1)- PowerShell Script to upload files using uploadserver module
 [Powerup.ps1](https://github.com/PowerShellMafia/PowerSploit/blob/master/Privesc/PowerUp.ps1)- priv escalation enumerator