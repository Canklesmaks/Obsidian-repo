...
```shell-session
canklesmaks@htb[/htb]$ gem install evil-winrm
```
...
```shell-session
canklesmaks@htb[/htb]$ evil-winrm 
...
```shell-session
canklesmaks@htb[/htb]$ evil-winrm -i 10.129.201.234 -u forend
...
```shell-session
canklesmaks@htb[/htb]$ mssqlclient.py 
...
```shell-session
canklesmaks@htb[/htb]$ mssqlclient.py INLANEFREIGHT/DAMUNDSEN@172.16.5.150 -windows-auth
