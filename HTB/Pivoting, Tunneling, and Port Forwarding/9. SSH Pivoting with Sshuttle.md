...
* Let's install sshuttle via our Ubuntu pivot host and configure it to connect to the Windows host via RDP.
...
```shell-session
canklesmaks@htb[/htb]$ sudo apt-get install sshuttle
...
```shell-session
canklesmaks@htb[/htb]$ sudo sshuttle -r ubuntu@10.129.202.64 172.16.5.0/23 -v 
...
```shell-session
canklesmaks@htb[/htb]$ nmap -v -sV -p3389 172.16.5.19 -A -Pn
