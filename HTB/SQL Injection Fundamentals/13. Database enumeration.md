...
As an initial guess, 
* if the webserver we see in HTTP responses is `Apache` or `Nginx`, it is a good guess that the webserver is running on Linux, 
	* so the DBMS is likely `MySQL`. 

* The same also applies to Microsoft DBMS if the webserver is `IIS`, 
	* so it is likely to be `MSSQL`. However, this is a far-fetched guess, as many other databases can be used on either operating system or web server. 

So, there are different queries we can test to fingerprint the type of database we are dealing with.
...
To dump the data of the `credentials` table, 
