...
Just like any other language, SQL allows the use of comments as well. 
* Comments are used to document queries or ignore a certain part of the query. 
...
Note: In SQL, using two dashes only is not enough to start a comment. 
* So, there has to be an empty space after them, so the comment starts with (-- ), with a space at the end. 
* This is sometimes URL encoded as (--+), as spaces in URLs are encoded as (+). 
* To make it clear, we will add another (-) at the end (-- -), to show the use of a space character.
