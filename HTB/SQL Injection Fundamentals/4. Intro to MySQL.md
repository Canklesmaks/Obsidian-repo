...
```shell-session
canklesmaks@htb[/htb]$ mysql -u root -p
...
```shell-session
canklesmaks@htb[/htb]$ mysql -u root -p<password>
...
```shell-session
canklesmaks@htb[/htb]$ mysql -u root -h docker.hackthebox.eu -P 3306 -p 
...
The SQL queries above create a table named `logins` with four columns. The first column, `id` is an integer. The following two columns, `username` and `password` are set to strings of 100 characters each. Any input longer than this will result in an error. The `date_of_joining` column of type `DATETIME` stores the date when an entry was added.Intro to MySQL
